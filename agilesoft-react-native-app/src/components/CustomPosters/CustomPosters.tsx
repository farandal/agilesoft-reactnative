...
import { IAgileSoftMovie, IAgileSoftMovieResults } from '../../utils/interfaces';
  ...
  const renderItem:FC<{item:IAgileSoftMovie}> = ({ item }) => {
    return (
      <TouchableOpacity onPress={onPress ? (e) => { onPress(item,imageBaseUrl) } : (e) => { console.log(item) }} style={[estilos.itemContainer]} >
        <Image style={[Layout.fullSize]} height={ITEM_HEIGHT} source={{uri:`${imageBaseUrl}${item.poster_path}`}} resizeMode={mode} />
      </TouchableOpacity>
      );
    }
